...
WE SHOULD JUST START WITH RUNNING THE MODEL ON A SINGLE INDIVIDUAL... THEN WE CAN DO OVERLAP
```{r Telemtry}
# Creating ctmm telemetry object (can be read directly from movebank)
coyotes <- as.telemetry(data)

#pulling out specific inidividual
coy1 <- coyotes$IdCoy_P4_2
...
plot(coy1)
```
We can see here that this coyote appears to be *range resident* with no obvious migrations. Now we can calculate and plot the coyote's variograms.
