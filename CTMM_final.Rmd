...
Follow the steps outlined above to calculate guesstimate parameters of our second individual, `coy2`, for model selection and testing.

- When considering your `coy2` plot from earlier and fit variogram, how do the visualization of `coy1` movement differ?
...
AICc is the (linearly) corrected Akaike information criteria. AIC balances likelihood against model complexity in a way that is good if we want to make optimal predictions. Recall from [Module 16 - Model Selection in General Linear Regression](https://fuzzyatelin.github.io/bioanth-stats/module-16/module-16.html) that AIC helps us to determine which model has the best fit. This metric is typically calculated via  *-2(log-likelihood) + 2K* with the log-likelihood being a measure of a model's fit and K is the number of parameters within the model. In addition to assessing fit, AICc typically penalizes model complexity to encourage the removal of unnecessary predictor variables. While AIC does not tell us how good a model is on its own, it is useful in comparisons like these to assess fit. Here, OUF would be the better model.
...
Plotting the AKDE estimate defaults to show the 95% home range contour and corresponding 95% confidence intervals. The summary produces point estimates and confidence intervals. Other confidence intervals can be specified if desired.
...
```{r names}
par(mfrow = c(2, 2))
plot(variograms[[1]],CTMM=coyotes_FITS,col.CTMM=c("purple"),fraction=0.65,level=0.5)
plot(variograms[[1]],CTMM=coyotes_FITS,col.CTMM=c("purple"),fraction=0.05,level=0.5)
plot(variograms[[2]],CTMM=coyotes_FITS,col.CTMM=c("blue"),fraction=0.65,level=0.5)
plot(variograms[[2]],CTMM=coyotes_FITS,col.CTMM=c("blue"),fraction=0.05,level=0.5)
```
